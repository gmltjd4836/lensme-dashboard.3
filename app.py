...
# 🌟🌟🌟 발급받은 API 키 2개를 이곳에 넣어두시면 매번 입력할 필요가 없습니다! 🌟🌟🌟
DATA_GO_KR_API_KEY = "여기에_소상공인_인증키를_붙여넣으세요"
KAKAO_REST_API_KEY = "여기에_카카오_REST_API키를_붙여넣으세요"
...
st.sidebar.markdown("매장명이나 지하철역, 건물명을 검색하세요.")
...
if st.sidebar.button("🚀 지도로 이동하기", use_container_width=True):
    if not KAKAO_REST_API_KEY or "여기에_카카오" in KAKAO_REST_API_KEY:
        st.sidebar.error("⚠️ 상호명 검색을 위해 코드 상단에 카카오 REST API 키를 넣어주세요.")
    elif search_query:
        ...
        if lat and lon:
            ...
            st.sidebar.error(f"'{search_query}'(을)를 찾을 수 없습니다. 다르게 검색해보세요.")

st.sidebar.markdown("---")
current_store = st.sidebar.text_input("현재 기준 매장명", value="렌즈미 천안쌍용점")
st.sidebar.text_input("현재 분석중인 타겟 상권", value=st.session_state.candidate_store, disabled=True)

st.sidebar.markdown("---")
st.sidebar.info("💡 카카오 키(위치검색)와 소상공인 키(경쟁사분석)가 모두 세팅되면 완벽하게 작동합니다.")
...
with tab_map:
    ...
    folium.Marker(
        ...
        popup=folium.Popup(f"<b>{st.session_state.candidate_store}</b> (이전 후보지)", max_width=300),
    ...
    if competitors is None:
        ...
        st.warning("⚠️ 소상공인 API 키가 입력되지 않아 임시(샘플) 마커를 표시합니다.")
    else:
...
# (탭 2: 유동인구, 탭 3: 레이더차트, 탭 4: ROI 부분은 이전 코드와 동일하게 유지)
...
with tab_radar:
    ...
    with col2:
        ...
        fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 100])), showlegend=True, margin=dict(t=30, b=30, l=30, r=30))
...
with tab_roi:
    ...
    with col_result:
        ...
        with c2: st.markdown(f'<div class="metric-box"><div class="metric-title">예상 월 수익</div><div class="metric-value" style="color:{"#ef4444" if monthly_net_profit<=0 else "#10b981"}">{int(monthly_net_profit):,}만</div></div>', unsafe_allow_html=True)
        ...
        if monthly_net_profit > 0:
            ...
            fig_bar = px.bar(df_roi, x='월(Month)', y='누적 수익', title=f"⏳ {st.session_state.candidate_store} 누적 수익 예측", color='누적 수익', color_continuous_scale=px.colors.diverging.RdYlGn)
